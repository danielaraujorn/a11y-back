defmodule AccessibilityReporter.Accounts do
  @moduledoc """
  ...
  alias AccessibilityReporter.Accounts.Schema.{User, UserToken}
  alias AccessibilityReporter.Accounts.UserNotifier
  ...
  def get_user_and_deficiencies(id) do
    query_preload = select(Deficiency, [d], map(d, [:id, :name, :description, :place_id]))

    Repo.preload(get_user(id), deficiencies: query_preload)
  end

  def get_users(params) do
    user_fields = User.__schema__(:fields)
    deficiencies_fields = [deficiencies: [:id, :name, :description, :place_id]]
    select_fields = user_fields ++ deficiencies_fields

    %{
      entries:
        User
        |> join(:left, [u], d in Deficiency, on: d.user_id == u.id)
        |> preload([u, d], deficiencies: d)
        |> select([u, d], map(u, ^select_fields))
        |> DataManipulation.apply_operations(params)
        |> Repo.all(),
      total: User |> select([d], count(d.id)) |> Repo.one()
    }
  end
  ...
  def update_user_role(id, role) do
    if user = get_user(id) do
      user
      |> Changeset.cast(%{id: id, role: role}, [:id, :role])
      |> Repo.update()
    end
  end
